...
# Number of capitalised words
    ...
    summary_numbers = []
    length_words = {}
    for letters, u_words, l_words, numbers, sum_numbers, len_words in cleaned_words:
        if letters.istitle() == True:
            big_letters.append(letters)
        print(f"There are {len(big_letters)} titlecase words.")
        if u_words.isupper() == True and not u_words.startswith("30N"):
            upper_words.append(u_words)
        print(f"There are {len(upper_words)} uppercase words.")
        if l_words.islower() == True:
            lower_words.append(l_words)
        print(f"There are {len(lower_words)} lowercase words.")
        if numbers.isnumeric() == True:
            summary_figures.append(numbers)
        print(f"There are {len(summary_figures)} numeric strings.")
        if sum_numbers.isnumeric() == True:
            i_numbers = int(sum_numbers)
